...
async def sync_task(user: models.User):
    ...
    logger.info(
        f"{context.rid_get()} sync manager user {user.id} bucket '{bucket_name}' orgs '{org_names}', cache dir '{cache_dir}' starting"
    )
    ...
    while True:
        for org in org_names:
            ...
            download_result = services.storage.gcs.sync_download(
            ...
            if download_result.files_synced:
                logger.info(
                    f"{context.rid_get()} sync manager org '{org}' - files downloaded {len(download_result.files_synced)}"
                )

            if download_result.files_deleted:
                logger.info(
                    f"{context.rid_get()} sync manager org '{org}' - files deleted {len(download_result.files_deleted)}"
                )

            if sync_counter % 5 == 0:
                # sync upload - local fs changes to gcs
                upload_result = services.storage.gcs.sync_upload(
                    bucket_name=bucket_name,
                    org_name=org,
                    cache_dir=cache_dir,
                    sync_mode="rw",
                ...
                if upload_result.files_synced:
                    logger.info(
                        f"{context.rid_get()} sync manager org '{org}' - files uploaded {len(upload_result.files_synced)}"
                    )
