...
def create_agent_places(
    model: pydantic_ai.models, output_types: list
) -> pydantic_ai.Agent:
    agent = pydantic_ai.Agent(
        deps_type=dict,
        model=model,
        output_type=output_types,
        system_prompt=(
            "You are a helpful agent. Use one of the following tools to help compose your answer."
        ),
        tools=[],
    )
...
def model_gemini() -> pydantic_ai.models:
    return pydantic_ai.models.google.GoogleModel(
        ...
        provider=pydantic_ai.providers.google.GoogleProvider(
            api_key=os.getenv("GOOGLE_GEMINI_KEY"), vertexai=False
        ),
    )
